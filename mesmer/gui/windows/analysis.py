...
from mesmer.lib.setup_functions	import open_user_prefs
...
class AnalysisWindow(tk.Frame):
	...
	def loadPrefs(self):
		...
		self.plot_plugins = load_plugins(self.prefs, 'gui_p', disabled_writeback=True )
	...
	def createWidgets(self):
		...
		self.progressPlotButton.grid(column=0,row=2,padx=(6,0),pady=(0,4),sticky=tk.W)
		...
		self.attributePlotButton.grid(column=0,row=1,padx=(6,0),sticky=tk.W)
		...
		self.fitPlotButton.grid(column=0,row=1,padx=(6,0),pady=(0,4),sticky=tk.W)

		self.f_footer = tk.Frame(self.container)
		self.f_footer.grid(column=0,row=6,columnspan=3)
