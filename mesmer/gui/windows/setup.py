...
class SetupWindow(tk.Frame):
	def __init__(self, master, parent):
		...
		setControlVarsFromArgs(self, args)
	...
	def createWidgets(self):
		...
		self.targetFilesScroll.grid(column=2,row=3,rowspan=2,sticky=tk.W+tk.N+tk.S,pady=4)
		...
		self.componentFilesList.grid(column=1,row=5,rowspan=2,sticky=tk.W,padx=2,pady=4)
		...
		self.componentFilesScroll.grid(column=2,row=5,rowspan=2,sticky=tk.W+tk.N+tk.S,pady=4)
		...
		self.maxGenerationsBox.grid(column=2,row=2,sticky=tk.W)
		...
		self.componentStatsBox.grid(column=1,row=1,sticky=tk.W)
		...
		self.optToleranceEntry.grid(column=1,row=2,sticky=tk.W)
		self.optIterationsEntry = tk.Entry(self.f_optimization,width=4,textvariable=self.optIterations)
		self.optIterationsEntry.grid(column=1,row=3,sticky=tk.W)
		...
		self.lConfigButton.grid(column=0,row=0,sticky=tk.S)
		self.sConfigButton = tk.Button(self.f_buttons, text='Save Config...', command=lambda:saveControlVarArgs(self) )
		self.sConfigButton.grid(column=0,row=1,sticky=tk.N)
		...
		self.startButton.grid(column=0,row=2,pady=4,sticky=tk.S)
		self.closeButton = tk.Button(self.f_buttons, text='Cancel', command=self.close)
		self.closeButton.grid(column=0,row=3,pady=4,sticky=tk.N)
	...
	def loadControlVarArgs():
		tmp = tkFileDialog.askopenfilename(title='Select MESMER run config file:',parent=w,initialdir=self.prefs['last_open_dir'])
		if(tmp == ''):
			return
		self.prefs['last_open_dir'] = os.path.dirname(tmp)
		args = parse_arguments(["@%s"%(tmp)],self.prefs)
		setControlVarsFromArgs(w, args)

	def saveControlVarArgs(w):
		text = makeStringFromArgs( getArgsFromWindow(w) )
		if(text == None):
			return
	...
	def setControlVarsFromArgs(w, args):
	...
	def getArgsFromWindow( w ):
		...
		args = parse_arguments( pre_args, self.prefs )
		args.name = self.runTitle.get().replace(' ','_')
		
		args.dir = self.saveResults.get()
		args.target = [os.path.normpath(os.path.join(self.basedir,f)) for f in list(self.targetFilesList.get(0,tk.END))] #see bug notice in createControlVars
		args.components = [os.path.normpath(os.path.join(self.basedir,f)) for f in list(self.componentFilesList.get(0,tk.END))] #see bug notice in createControlVars
		args.size = self.ensembleSize.get()
		args.ensembles = self.numEnsembles.get()
		args.Gcross = self.gCrossFreq.get()
		args.Gmutate = self.gMutateFreq.get()
		args.Gsource = self.gSourceRatio.get()
		if(self.minFitnessCheck.get()>0):
			args.Fmin = self.minFitness.get()
		else:
			args.Fmin = None
		if(self.minRSDCheck.get()>0):
			args.Smin = self.minRSD.get()
		else:
			args.Smin = None
		if(self.maxGenerationsCheck.get()>0):
			args.Gmax = self.maxGenerations.get()
		else:
			args.Gmax = None
		args.Pbest = (self.bestFitCheck.get()>0)
		args.Popt = (self.optimizationStateCheck.get()>0)
		args.Pextra = (self.pluginExtrasCheck.get()>0)
		if(self.componentStatsCheck.get()>0):
			args.Pstats = True
			args.Pmin = self.componentStats.get()
		else:
			args.Pstats = False
			args.Pmin = None
		if(self.componentCorrCheck.get()>0):
			args.Pcorr = self.componentCorr.get()
		else:
			args.Pcorr = 100
		try:
			args.Ralgorithm = self.optMethodOptions.index(self.optMethodOption.get())
		except:
			args.Ralgorithm = 0
		args.Rprecision = self.optTolerance.get()
		args.Rn = self.optIterations.get()
