...
class GetEnsembleStats(MESMERUtility):
	def __init__(self):
		super(GetEnsembleStats, self).__init__()
		
		group0 = self.parser.add_argument_group('Input options')
		...
		group1 = self.parser.add_argument_group('Output options')
		...
		group2 = self.parser.add_argument_group('Ordering options')
	...
	def run(self,args):
		...
		if( not args.table ):
			return 1,"ERROR:\tMust specify an attribute table."

		if( not args.state ):
			return 1,"ERROR:\tMust specify a state table."
		...
		try:
			...
			return 1,"ERROR:\tCouldn't read component names from column %i of the attribute table" % args.nCol
		...
		for col in args.dCols:
			try:
				...
				return 1,"ERROR:\tCouldn't read component values from column %i of the attribute table" % i

		try:
			ensembles = get_ensembles_from_state( args.state, unique=True )
		except Exception as e:
			return 1,"ERROR:\tCouldn't read ensembles from state file. Reason: %s" % (e)
			...
			for col in range(len(args.dCols)):
				...
				for name in e['components']:
					try:
						...
						return 1,"ERROR:\tCould not find component \"%s\" in the attribute table" % name
		...
		if( args.Pall ):
			...
			if( len(args.dCols) > 1 ):
				return 1,"For sorting by average attribute, specify a single attribute column."
			sortable = average_attributes

		elif( args.deviation ):
			if( len(args.dCols) > 1 ):
				return 1,"For sorting by deviation from a specified value, specify a single attribute column."
