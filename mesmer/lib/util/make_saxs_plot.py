...
class MakeSAXSPlot(MESMERUtility):
	def __init__(self):
		super(MakeSAXSPlot, self).__init__()
		...
		group1.add_argument('-xLabel',					default=r'$q (A^{-1})$',				help='Label for the X axis')
		group1.add_argument('-yLabel',					default='I(q), (intensity)',			help='Label for the Y axis')
	...
	def run(self,args):
		...
		b = plot.axes([0.55,0.55,0.38,0.38])
		for (i,d) in enumerate(data):
			if len(data) == 1:
				b.plot( d[0], [1]*len(d[0]), '%s-' % args.colors[i], linewidth=2 )
				b.scatter( d[0], d[1]/d[2], edgecolor='k', facecolors='none' )
			else:
				b.plot( d[0], [1]*len(d[0]), 'k-', linewidth=2 )
