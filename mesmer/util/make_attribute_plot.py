...
class MakeAttributePlot(MESMERUtility):
	def __init__(self):
		super(MakeAttributePlot, self).__init__()
		
		group0 = self.parser.add_argument_group('Table specification')
		group0.add_argument('tables', 	nargs='+',		metavar='<FILE>.tbl',		help='File(s) containing tab-delimited statistics of the available components #GUI')
		group0.add_argument('-nCol',	default=0,		metavar='0',	type=int,	help='The column containing the component names #GUI')
		group0.add_argument('-xCol',	default=1,		metavar='1',	type=int,	help='The column containing the desired component attribute #GUI')
		group0.add_argument('-yCol',	default=2,		metavar='2',	type=int,	help='The column to use as y-axis data #GUI')
		group0.add_argument('-zCol',	default=None,	metavar='3',	type=int,	help='The column to use as z-axis data #GUI')
		...
		group1.add_argument('-title',					default='',				help='The title for the output figure #GUI')
		group1.add_argument('-xLabel',					default='',				help='The label for the x-axis #GUI')
		group1.add_argument('-yLabel',					default='',				help='The label for the y-axis #GUI')
		group1.add_argument('-zLabel',					default='',				help='The label for the z-axis #GUI')
		...
		group1.add_argument('-axes',		nargs='+',	default=None,				metavar='N',	help='The scales for the axes: xmin, xmax, ymin, ymax #GUI')
		...
		group2 = self.parser.add_argument_group('Component statistic overlay options')
		...
		group2.add_argument('-statNorm',	action='store_true', default=False,				help='Normalize variable color saturation for component prevalence #GUI')
		group2.add_argument('-statSame',	action='store_true', default=False,				help='Do not use variable color saturation for component prevalence #GUI')
		group2.add_argument('-statWeight',				default=-1.0,	type=float,			help='When plotting MESMER statistics, drop components weighted lower than this amount #GUI')
		group2.add_argument('-statPrevalence',			default=-1.0,	type=float,			help='When plotting MESMER statistics, drop components with prevalences lower than this amount #GUI')
		group2.add_argument('-statSubsample',			default=1,		type=float,			help='Randomly subsample selected conformers by a percentage. #GUI')

		group3 = self.parser.add_argument_group('Ensemble statistics overlay options')
		...
		group4 = self.parser.add_argument_group('Synthetic target specification overlay options')
		...
		group5 = self.parser.add_argument_group('Highlighting overlay options')
		...
		group6 = self.parser.add_argument_group('Output options')
	...
	def _make_named_vectors( self, tbl ):
	...
	def run(self,args):
		if(len(args.ensembleHighlight)>0 and not args.ensembles):
			return 1,"ERROR:\tTo highlight a specific ensemble ID please provide an ensemble state file."

		plot_data = [ [],[],[],[] ] # generate master data lists
		for t in args.tables:
			if( args.zCol ):
				tmp = self._make_named_vectors(scipy.genfromtxt( t, usecols=(args.nCol,args.xCol,args.yCol,args.zCol), dtype=str, unpack=True ))
			else:
				tmp = self._make_named_vectors(scipy.genfromtxt( t, usecols=(args.nCol,args.xCol,args.yCol,args.yCol), dtype=str, unpack=True ))
		...
		if( args.spec ):
			spec_data = self._make_named_vectors(scipy.genfromtxt( args.spec, usecols=(0,1), dtype=str, unpack=True ))
			spec_data.extend( [[],[],[]] )
			for (i,n) in enumerate(spec_data[0]): # check names against provided component names and retrieve coordinates

				if(args.specLog):
					spec_data[1][i] = 1/(-1.0 * scipy.log(spec_data[1][i]))

				try:
					k = plot_data[0].index(n)
				except:
					return 1,"ERROR:\tTarget specification contains a component (%s) that does not exist in the attribute table." % n
		...
		if( args.stats ):
			...
			while(line):
				...
				try:
					...
					return 1,"ERROR:\tEnsemble statistics file contains a component (%s) that does not exist in the attribute table." % a[0]
			...
			for (i,id) in enumerate(id_list):
				for (j,name) in enumerate( ensemble_stats[id]['components'] ):
					try:
						...
						return 1,"ERROR:\tThe component \"%s\" from the ensemble state file doesn't exist in the attribute table." % (name)
		...
		if( len(args.highlight) > 0 ):
			...
			for name in args.highlight:
				try:
					...
					return 1,"ERROR:\tThe component \"%s\" from highlight list doesn't exist in the attribute table." % (name)
