...
#NOTE: Function name read as: PCS(1M)ODELS(1S)SITE(F)IXED(M)ETAL(F)IXED(T)ENSOR
def PCS1M1SFMFT(p0, meas, Dax, Drh, xm,ym,zm, x,y,z, tol=None):
    """
     Optimize for the X-tensor given a single model with fixed metal position and fixed Dax Drh
     @param p0: A list of initial estimates for the 5 unknown X-tensor params
    """
    a, b, g = p0
    rot  = ZYZRot(a, b, g)
...
def RDC1M1Sfix(p0, meas_rdc, Dax, Drh, x1,y1,z1, x2,y2,z2, scal, tol=None):
	# fixed Dax and Drh
    a, b, g = p0
    rot  = ZYZRot(a, b, g)
    X    = x1 - x2
...
def RDC1M1SABG(p0, meas_rdc, a, b, g, x1,y1,z1, x2,y2,z2, scal, tol=None):
	# fixed Dax and Drh
    Dax, Drh = p0
    rot  = ZYZRot(a, b, g)
    X    = x1 - x2
