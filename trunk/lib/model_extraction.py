...
def _interpolate_color( value, start, end ):

	rA,rB = cmd.get_color_tuple(start), cmd.get_color_tuple(end)
	hA,hB = colorsys.rgb_to_hsv( rA[0], rA[1], rA[2] ), colorsys.rgb_to_hsv( rB[0], rB[1], rB[2] )
	...
	return colorsys.hsv_to_rgb( ret[0], ret[1], ret[2] )

def _make_<<NAME>>_colors( start_color='blue', end_color='red' ):
	global _M<<NAME>>Values
	global _M<<NAME>>Colors

	\"""
	Generate a color scheme from MESMER data. Apply the color scheme with MES_<<NAME>>_apply
	Usage: MES_<<NAME>>_color [start_color] [end_color]
	\"""

	_M<<NAME>>Colors = []
	for (i,w) in enumerate(_M<<NAME>>Values):
		cmd.set_color( "mWeightColor_%i"%i, _interpolate_color(w, start_color, end_color) )
		_M<<NAME>>Colors.append( "mWeightColor_%i"%i )

def _apply_<<NAME>>_colors( model='*', repr="cartoon_color" ):
	global _M<<NAME>>Colors

	\"""
	Apply a color scheme generated by MES_<<NAME>>_color, or use default color scheme
	Usage: MES_<<NAME>>_apply [model name] [representation]
	\"""

	if len(_M<<NAME>>Colors) == 0:
		make_<<NAME>>_colors()

	cmd.set("all_states", "on")
	for i in range(len(_M<<NAME>>Colors)):
		cmd.set(repr, _M<<NAME>>Colors[i],model,i)

cmd.extend("MES_<<NAME>>_color", _make_<<NAME>>_colors)
cmd.extend("MES_<<NAME>>_apply", _apply_<<NAME>>_colors)

""" % (values)

	f = open( path, 'w' )
	f.write( script.replace('<<NAME>>',name) )
	f.close()
