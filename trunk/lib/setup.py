...
def read_param_file( path ):
	"""
	Returns a dictionary of parsed MESMER parameters from a provided configuration file
	"""
	...
	group4.add_argument('-Pmin',		action='store',		default=1.0,	type=float,		metavar='F',	help='Print final conformer ratio state only if they exist in this percentage of ensembles or greater.')
...
def load_all_components( args, plugins, targets ):
	...
	if( args.dbm ):
		...
		print_msg("INFO: Loading %i component files into memory." % (len(files)))
		components = {}
