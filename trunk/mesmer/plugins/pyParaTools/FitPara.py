...
class FitPara:
    ...
    def PCS(self, parsedObj, type_of_fit, parsedObj2="None"):
        ...
        if type_of_fit == 7:
            p0 = zeros(3)
            p0[0], p0[1], p0[2]  = a, b ,g
            soln,cov,info,mesg,success = leastsq(PCS1M1SFMFT, p0, args=(meas, ax, rh, met[0], met[1], met[2], x,y,z), full_output=1)
            qual = FitSummary(soln,cov,info,mesg,success,p0, meas, type_of_fit)
            self.__UpdateFromFit(parsedObj, type_of_fit, soln)
    ...
    def RDC(self, parsedObj, type_of_fit, scal, parsedObj2="None"):
        ...
        meas  = parsedObj.getAllMeasarray()
        Dax   = ToVVU(parsedObj.getAxial())
        Drh   = ToVVU(parsedObj.getRhombic())
        a     = parsedObj.getAlpha()
        b     = parsedObj.getBeta()
        g     = parsedObj.getGamma()
        ...
        if type_of_fit == 0:
        	...
        	# 1 Model, 3 params (a,b,g)
            p0 = zeros(3)
            p0[0], p0[1], p0[2] = a, b, g
            soln,cov,info,mesg,success = leastsq(RDC1M1Sfix, p0, \
                                              args=(meas, Dax, Drh, x1,y1,z1, \
                                                          x2,y2,z2, scal),\
                                                          full_output=1)

        elif type_of_fit == 2:
        	# 1 Model, 2 params (Dax,Drh)
            p0 = zeros(2)
            p0[0], p0[1] = Dax, Drh
            soln,cov,info,mesg,success = leastsq(RDC1M1SABG, p0, \
                                              args=(meas, a, b, g, x1,y1,z1, \
                                                          x2,y2,z2, scal),\
                                                          full_output=1)

        RDCFitSummary(soln,cov,info,mesg,success,p0, meas, type_of_fit)
#       print "Dax: %s (%s,%s)" % (soln[0],ToVVU(soln[0]),FromVVU(soln[0]))
#       print "Drh: %s (%s,%s)" % (soln[1],ToVVU(soln[1]),FromVVU(soln[1]))
    ...
    def __UpdateFromFit(self, parsedObj, type_of_fit,fitRes, parsedObj2="None"):
        ...
        if parsedObj.getDataType() == 'pcs':
            ...
            if type_of_fit == 1:
                ...
                self.__AnglesUTR(parsedObj)
            if type_of_fit == 7:
                parsedObj.setAlpha(FixAngle(fitRes[0]))
                parsedObj.setBeta(FixAngle(fitRes[1]))
                parsedObj.setGamma(FixAngle(fitRes[2]))
                self.__AnglesUTR(parsedObj)
            if type_of_fit == 2:
                ms = zeros(3)
                ms[0], ms[1], ms[2] = fitRes[0], fitRes[1], fitRes[2]
